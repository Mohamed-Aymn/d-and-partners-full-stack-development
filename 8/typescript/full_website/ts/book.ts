...
const found = id ? getBook(id) : null;
...
if (!root) {
  ...
  const range = document.querySelector<HTMLInputElement>('#progress-range');
  ...
  const notes = document.querySelector<HTMLElement>('#notes-text');
